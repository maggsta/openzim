...
class OdfTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testSetImageInSegment()
    {
        $loop1 = $this->odf->setSegment('loop1');
		for ($j = 0; $j < 3; $j++) {
			$loop1->setImage('testloop1', self::TEST_PATH . '/php_elephant.gif');
			$loop1->merge();
		}
        $this->odf->mergeSegment($loop1);
    ...
    public function testGetExistantConfig()
    {
		$this->assertEquals('PclZipProxy', $this->odf->getConfig('ZIP_PROXY'));
		$this->assertEquals('{', $this->odf->getConfig('DELIMITER_LEFT'));
		$this->assertEquals('}', $this->odf->getConfig('DELIMITER_RIGHT'));
    }
    ...
    {
        $this->odf->setVars('societe', 'anaska');
        $this->assertRegExp('/' . $this->odf->getConfig('DELIMITER_LEFT') . 'societe' . $this->odf->getConfig('DELIMITER_RIGHT') . '/', $this->odf->printVars());
        $this->assertRegExp('/anaska/', $this->odf->printVars());
    }
}
